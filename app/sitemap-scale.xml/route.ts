...
const CATEGORY = 'scale' as const;

const LOCALE: Locale = 'en';

export async function GET() {
  const scales = await getAllScales();
  const eligibleScales = filterEligibleSitemapEntries(
    scales,
    CATEGORY,
    LOCALE,
    scale => `/scale/${scale.slug}`,
    scale => scale.lastUpdated
  );
  ...
  const scaleEntries = eligibleScales.map(scale =>
