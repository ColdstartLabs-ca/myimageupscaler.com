...
const CATEGORY = 'device-optimization' as const;
...
export async function GET() {
  ...
  const eligiblePages = filterEligibleSitemapEntries(
    ...
    CATEGORY,
    'en',
    page => `/device-optimization/${page.slug}`,
...
${generateSitemapHreflangLinks('/device-optimization', CATEGORY).join('\n')}
  </url>
${eligiblePages
  .map(
    page => `  <url>
    ...
    <changefreq>weekly</changefreq>
    <priority>0.8</priority>
${generateSitemapHreflangLinks(`/device-optimization/${page.slug}`, CATEGORY).join('\n')}${
      page.ogImage
        ? `
    <image:image>
