...
**Release decision (2026-09-08): require durable interrupted-download recovery, not a two-second disconnect callback.**

The user delegated the release decision after deferring immediate disconnect handling. The production contract is now explicit: a disconnected client must not falsely complete the job; its delivery lock must expire within three minutes; the same job must then deliver a valid image without another prediction, debit or refund; and the old lease owner cannot release or acknowledge the replacement owner's delivery. Prompt cancellation cleanup remains best effort and keeps its unit coverage. This accepts a possible three-minute retry delay, not lost work or duplicate billing.

The real Worker/database integration test now interrupts an HTTP download, checks the bounded persisted lock and absent acknowledgement, advances only the disposable database lease clock, retries through the real output route, rejects stale-owner release/acknowledgement, and checks image dimensions, exact ledger rows and balances. All five release projects remain mandatory under `yarn deploy`, including `--skip-tests`. A fresh full gate must pass; no project is excluded. Production backup, migration, credential and deployment guards remain unchanged.

Historical failure evidence: `/tmp/upscale-release-disconnect-fixed.log` recorded 13 passed, one failed and five skipped against the former two-second cleanup requirement. Signal propagation was not proven by three attempted fixes. This is not evidence of immediate callback reliability and must not be presented as such.
