...
4. **Fix the PR**:
   ...
   a. **Create an isolated review worktree**:
      ```
      git fetch origin
      git worktree add --detach ../vite-react-typescript-starter-nw-review-<branch-name> origin/master
      ```

   b. **Check out the PR branch inside that worktree**:
      ```
      cd ../vite-react-typescript-starter-nw-review-<branch-name>
   ...
   i. **Clean up worktree**: `git worktree remove ../vite-react-typescript-starter-nw-review-<branch-name>`

5. **Repeat** for all open PRs that need work.

6. When done, return to master: `git checkout master`
