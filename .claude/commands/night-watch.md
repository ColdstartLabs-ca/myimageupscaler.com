...
4. **For each PRD** (process ONE at a time, then stop):
   ...
   git worktree add -b night-watch/<prd-filename-without-.md> ../vite-react-typescript-starter-nw-<prd-name> origin/master
   ```
   ...
   j. **Move PRD to done** (back in main repo on master):
   ...
   l. **Commit** the move + summary update, push master.

   m. **Clean up worktree**: `git worktree remove ../vite-react-typescript-starter-nw-<prd-name>`
