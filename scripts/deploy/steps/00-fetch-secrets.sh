...
step_fetch_secrets() {
    ...
    if ! gcloud auth print-identity-token --account="$GCLOUD_ACCOUNT" &> /dev/null; then
        log_error "Not authenticated as $GCLOUD_ACCOUNT. Run: gcloud auth login"
    fi
    ...
    if ! gcloud secrets versions access latest --secret="$GCLOUD_SECRET_API" --project="$GCLOUD_PROJECT" --account="$GCLOUD_ACCOUNT" > "$ENV_API_PROD" 2>/dev/null; then
    ...
    if ! gcloud secrets versions access latest --secret="$GCLOUD_SECRET_CLIENT" --project="$GCLOUD_PROJECT" --account="$GCLOUD_ACCOUNT" > "$ENV_CLIENT_PROD" 2>/dev/null; then
