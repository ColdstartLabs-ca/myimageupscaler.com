...
export { notifyAutoTopUpFailure } from '@server/services/auto-top-up-notification.service';
...
export class AutoTopUpService {
      ...
      try {
        ...
        if (setting.charge_claim_id) {
          ...
          const { data: staleAttempt, error: staleAttemptError } = await supabaseAdmin
            ...
            .select('id, stripe_payment_intent_id, credits')
          ...
          if (staleAttempt?.stripe_payment_intent_id) {
            ...
            if (existingIntent.status === 'succeeded') {
              if (!staleAttempt.credits) {
                throw new Error('stale_auto_top_up_attempt_missing_credits');
              }
              const { data: finalized, error: finalizeError } = await supabaseAdmin.rpc(
                'finalize_auto_top_up_attempt',
                {
                  p_attempt_id: staleAttempt.id,
                  p_payment_intent_id: staleAttempt.stripe_payment_intent_id,
                  p_credits: staleAttempt.credits,
                }
              );
              if (finalizeError || finalized !== true) {
                throw new Error(
                  `Unable to reconcile succeeded auto top-up: ${finalizeError?.message || 'finalization was not applied'}`
                );
              }
              continue;
            }
            if (existingIntent.status === 'processing') continue;
        ...
        if (
          ...
          await this.pauseSetting(setting, 'invalid_pack_configuration', undefined, true);
          result.failed++;
          continue;
        }
        ...
        const { data: lease, error: leaseError } = await supabaseAdmin
          ...
          .maybeSingle();
        if (leaseError)
          throw new Error(`Unable to lease auto top-up setting: ${leaseError.message}`);
        if (!lease) {
          await supabaseAdmin
            .from('auto_top_up_attempts')
        ...
        try {
          ...
          if (paymentIntentId) {
            ...
            if (error) {
              throw new Error(`Unable to finalize auto top-up failure: ${error.message}`);
            }
            if (data === null || data === undefined) {
              // Another worker (usually the Stripe webhook) finalized this failure first.
              result.failed++;
              continue;
            }
            if (typeof data !== 'number') {
              throw new Error('Unable to finalize auto top-up failure: invalid result');
            }
            failures = data;
          } else {
          ...
          try {
            ...
            console.error('[AUTO_TOP_UP_EMAIL_FAILED]', notificationError);
          }
          result.failed++;
        }
      } catch (settingError) {
        ...
        const invalidConfiguration =
          message.includes('Unknown price ID') || message === 'invalid_pack_configuration';
        console.error('[AUTO_TOP_UP_SETTING_FAILED]', {
          userId: setting.user_id,
          error: message,
        });
        try {
          await this.pauseSetting(
            setting,
            invalidConfiguration ? 'invalid_pack_configuration' : message,
            setting.charge_claim_id ?? undefined,
            invalidConfiguration
          );
        } catch (pauseError) {
          console.error('[AUTO_TOP_UP_SETTING_PAUSE_FAILED]', pauseError);
        }
        result.failed++;
      }
    }
  ...
  private async pauseSetting(
    ...
    const { error } = await supabaseAdmin
      ...
      .update({
        ...
        enabled: disable ? false : failures < 3,
  ...
  private async notifyFailure(userId: string, failures: number, attemptId: string): Promise<void> {
    await notifyAutoTopUpFailure(userId, failures, attemptId);
  }
  ...
  private async flushFailureNotifications(): Promise<void> {
    ...
    for (const attempt of data ?? []) {
      ...
      try {
        ...
        console.error('[AUTO_TOP_UP_PENDING_NOTICE_FAILED]', notificationError);
      }
    }
  }
}
