...
export function isAutoModelCompatible(
  ...
  return model.supportedScales.length === 0 ? scale === 2 : model.supportedScales.includes(scale);
}
 ...
 */
export function getAutoEligibleModels<
  T extends Pick<IModelConfig, 'creditMultiplier' | 'supportedScales'>,
>(models: readonly T[], scale: AutoScale): T[] {
  return models.filter(model => model.creditMultiplier < 8 && isAutoModelCompatible(model, scale));
}
...
export function resolveAutoModel<
  T extends Pick<IModelConfig, 'id' | 'creditMultiplier' | 'supportedScales'>,
>(models: readonly T[], scale: AutoScale, recommendedModelId?: string): T | null {
  const eligibleModels = getAutoEligibleModels(models, scale);
  return eligibleModels.find(model => model.id === recommendedModelId) ?? eligibleModels[0] ?? null;
}
