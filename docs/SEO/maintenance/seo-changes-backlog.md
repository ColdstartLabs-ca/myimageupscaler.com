...
- Deployed + RETAINED: release gate PASS on `e364d0aa`; production deploy run `35156313835` (Worker `7e34e378-114d-4b0e-ace7-c0f3d969d1e9`) now serves `x-opennext-cache: HIT` for prerendered pages, 137/137 zero-5xx soak, and three comparable mobile runs show improved measured response medians (TTFB 976→627 ms, server response ~876→142 ms, observed LCP 1213→572 ms) with no regression; simulated LCP median 4694 ms is flat and does NOT meet the ≤2.5s lab target. ~0.9–1.15 s outliers remain and the variance cause is unproven (traffic AC-4 stays PARTIAL). Rollback: previous Worker `582e06b4-a5da-4c83-af49-e040d6c8413f`; an in-repo rollback reverts the cache commit **and** its enabled-assertion test together. Evidence: `/tmp/cache-live-report-current`, `/tmp/cache-production-decision-current`.
- Flagship fact correction (`/blog/best-free-ai-image-upscaler-2026-tested-compared`): **published to the production DB record** at `2026-09-16T21:27:52Z` (stored body hash matches the prepared payload; title/slug/meta/H1/tags unchanged) after the `yarn db:backup` → `yarn db:backups` → `gzip -t` gate on `backups/backup_2026-09-16_14-21-15.{schema,data}.sql.gz`. **Public HTML now fresh:** served with `x-opennext-cache: HIT` showing the "Compared in 2026" heading and the four corrected vendor facts (Let's Enhance 10 credits, Bigjpg 20/mo, Pixelcut 3/day, Img.Upscaler 50/mo); old "Tested"/"5 credits/month"/"5 images/day" absent. Prepared copy: [corrected body](../reviews/2026-09-16-flagship-corrected-content.md), [diff](../reviews/2026-09-16-flagship-correction.diff). GSC request-indexing remains a manual follow-up.

### Homepage credit-offer locale contract fixed (raw message key was rendering)

PRD: [PRD-revenue-growth-2026-09](../../PRDs/revenue-growth-2026-09.md). Deployed at `0fff2b59` and live; consented checkout journey still open.
...
- AC-2: homepage portion done — live public render in all 7 locales shows region-appropriate offers with 0 raw keys (`/tmp/homepage-deploy-report-current`). Remaining: the consented real-client checkout journey (revenue AC-2 partial).
...
### Flagship fact correction published + homepage/cache deploys + mobile origin/cache diagnosis

PRD: [PRD-traffic-recovery-2026-09](../../PRDs/traffic-recovery-2026-09.md) / [PRD-revenue-growth-2026-09](../../PRDs/revenue-growth-2026-09.md). Production DB content changed (flagship correction) and both `0fff2b59` (homepage locale) and `e364d0aa` (cache) are deployed to production.

Changes:

- **Published** the prepared correction for `/blog/best-free-ai-image-upscaler-2026-tested-compared` to the production DB record under the explicit owner GO at `2026-09-16T21:27:52Z`: four contradicted vendor figures (Let's Enhance, Bigjpg, Pixelcut, Img.Upscaler) corrected to vendor-stated policy, and the body heading "…Tested in 2026" → "…Compared in 2026" to match the page's own no-benchmark disclosure. Title, `seo_title`, H1, slug, description, tags unchanged; stored body SHA256 matches the prepared payload. Fresh backups `backups/backup_2026-09-16_14-21-15.{schema,data}.sql.gz` were listed and `gzip -t`-verified before the write. Package: [review note](../reviews/2026-09-16-flagship-fact-correction.md), [corrected body](../reviews/2026-09-16-flagship-corrected-content.md), [diff](../reviews/2026-09-16-flagship-correction.diff). Public HTML now serves the corrected copy (`x-opennext-cache: HIT`).
- Deployed the homepage credit-offer locale fix at `0fff2b59` (release gate PASS 19/19, `yarn verify` clean, ff-pushed). Live public render: all 7 locales show region-appropriate offers with 0 raw keys, stable title/H1 (`/tmp/homepage-deploy-report-current`; the run's only failure is the known GitHub-egress `/api/health` 403 artifact).
- Ran the bounded mobile origin/HTML-cache diagnosis: [record](../reports/2026-09-16-mobile-origin-cache-diagnosis.md). Variance reproduces (14 cookie-free mobile requests to `/`: 148 ms–1,986 ms, median 633 ms, 57% over the 400 ms target) but the **cause is NOT established** — the samples cannot separate per-request Worker work from routing, contention, or the OpenNext path; no `cf-cache-status` and no `Age` on HTML, but that absence alone proves neither cause nor applicability. The authoritative active config is `wrangler.json` with `wrangler deploy` (`main: .open-next/worker.js`); `wrangler.toml`'s `pages_build_output_dir` is stale. No dashboard Cache Rule or `Vary`-strip was approved: the same URL serves `text/html` and `text/x-component`, cache lookup precedes application guards, and auth/geo/router variants are not promised safe without a test. The mechanism is confirmed as OpenNext cache interception (E5), preview causal isolation **PASSED** (`v00b58dae` / buildId `2EC0kLBVQ5C96yr32yFuk`; `/tmp/traffic-preview-report-current`), and production now RETAINS the change with `x-opennext-cache: HIT` and a 137/137 zero-5xx soak. Traffic AC-4 stays PARTIAL (variance reproduced, TTFB cause open); no Google/revenue recovery is claimed.
