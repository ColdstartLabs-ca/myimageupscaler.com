...
| **Images**   | Cloudflare Images           | Built-in optimization, resizing, global CDN, managed hosting                    |
...
| **Locale**   | Global (locale-independent) | Blog content is the same across all locales; UI locale handled by parent layout |
...
- `reading_time` is computed on create/update by the API and stored in the database
- `updated_at` is automatically set via trigger on every update
- `published_at` is preserved when unpublishing (for historical tracking)
...
CREATE TABLE public.blog_media (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  cloudflare_image_id TEXT UNIQUE NOT NULL,  -- Cloudflare Images ID
  filename TEXT NOT NULL,
  original_filename TEXT NOT NULL,
  mime_type TEXT NOT NULL,
  size_bytes INTEGER NOT NULL,
  cdn_url TEXT NOT NULL,  -- Cloudflare Images delivery URL
  width INTEGER,
  height INTEGER,
  alt_text TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW() NOT NULL,
...
CREATE INDEX idx_blog_media_cf_image_id ON public.blog_media(cloudflare_image_id);
...
| `/api/blog/posts/[slug]/unpublish` | POST   | Revert to draft (preserves `published_at` for history) |
| `/api/blog/images/upload`          | POST   | Upload image to Cloudflare Images                      |
...
| File                                | Changes                                   |
| ----------------------------------- | ----------------------------------------- |
| `shared/config/env.ts`              | Add Blog API + Cloudflare Images env vars |
...
**Security Note:** Adding `/api/blog/*` to `PUBLIC_API_ROUTES` bypasses **user JWT authentication only**. It does NOT allow anonymous access. Every blog API route MUST call `verifyBlogApiAuth()` at the start of the handler to enforce API key authentication. The `PUBLIC_API_ROUTES` setting simply tells the global auth middleware to skip user session checks for these routes.
...
export async function uploadToCloudflareImages(
  ...
  // Allow public access without signed URLs (images are public blog content)
...
### 5.1.1 Content Sanitization (Security)

Since blog content is authored by an AI agent and stored as markdown, proper sanitization is **required** to prevent XSS attacks when rendering.

**Rendering Configuration:**

- Use `react-markdown` with `rehype-sanitize` for HTML sanitization
- Whitelist only safe HTML elements: `p, h1-h6, ul, ol, li, a, img, code, pre, blockquote, em, strong, table, thead, tbody, tr, th, td`
- Strip all event handlers (`onclick`, `onerror`, etc.)
- Validate `href` and `src` attributes (allow only `https://` and relative paths)
- Disallow `<script>`, `<iframe>`, `<object>`, `<embed>`, `<form>`

```typescript
// Example: app/[locale]/blog/[slug]/page.tsx
...
const sanitizeSchema = {
  ...
  attributes: {
    ...
    img: [...(defaultSchema.attributes?.img || []), ['src', /^https?:\/\//]],
  },
};
...
export async function getAllPublishedPosts(): Promise<IBlogPostMeta[]> {
  const { data, error } = await supabaseAdmin
    .from('blog_posts')
    .select(
      'slug, title, description, published_at, author, category, tags, featured_image_url, reading_time'
    )
    .eq('status', 'published')
...
+ // Note: This only returns { slug } because locale is handled by the parent
+ // layout's generateStaticParams. Blog posts are locale-independent (same
+ // content for all locales), only the UI chrome is localized.
...
**Recommendation**: Use ISR (Incremental Static Regeneration) with on-demand revalidation for instant publishing while maintaining static performance.

```typescript
// app/[locale]/blog/page.tsx
export const revalidate = 3600; // Background revalidation every hour as fallback
```

**On-Demand Revalidation**: The publish/unpublish endpoints should trigger revalidation to ensure instant visibility:

```typescript
// In POST /api/blog/posts/[slug]/publish
...
This ensures new posts appear immediately after publishing while still benefiting from static caching.
...
describe('Blog API', () => {
  describe('POST /api/blog/posts', () => {
    ...
    it('auto-calculates reading_time from content and stores in DB');
  });

  describe('PATCH /api/blog/posts/[slug]', () => {
    it('updates post fields');
    it('recalculates reading_time when content changes');
    it('preserves published_at when unpublishing');
  });
...
- [ ] Images can be uploaded and stored in Cloudflare Images
