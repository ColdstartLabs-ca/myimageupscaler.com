...
**Status:** IN PROGRESS (Phase 1 locale fix deployed and live: 7 locales render region-appropriate offers with 0 raw keys; AC-2 homepage portion done, consented-checkout portion still open — AC-2 partial; AC-4 five-journey stop review complete and evidence-backed; AC-6/AC-7 open; AC-8 partial)
**Flag:** POST-DEPLOY-EVALUATION-REQUIRED — consented checkout journey and weekly/day-14/day-30 reviews (AC-2 partial, AC-6, AC-7)
...
- [~] AC-2 [shared; actor: João, agent]: The locale fix and any justified AC-4 funnel fix are deployed through the authorized rollout; an unauthenticated public render in the supported locales shows the expected offer wording per region and no raw key, and a consented journey through the real client → modal → checkout path opens correctly with no fabricated paid transaction (any checkout change is functionally tested in Stripe test mode only). No funnel fix ships (AC-4 stop). **Homepage portion DONE:** the locale fix is committed at `0fff2b59`, release-gated 19/19, pushed, and live after the `0fff2b59` deploy — production render confirms all 7 locales show region-appropriate "3/5 welcome credits" offers with 0 raw `homepage.*Subtext` keys and stable title/H1 (`/tmp/homepage-deploy-report-current`). **Remaining (this AC stays PARTIAL):** the consented real-client → modal → checkout journey in Stripe-live with no fabricated transaction, which is not yet verified.
...
**Status:** DONE (AC-1); AC-2 partial — locale fix committed at `0fff2b59`, release-gated 19/19, pushed, and live (7 locales, region-appropriate offers, 0 raw keys); the consented checkout journey remains unverified
**ACs:** AC-1, AC-2
...
**Shipping progress 2026-09-16:** the fix is committed at `0fff2b59`, passed the local release gate (`yarn test:upscale:release` 19/19, no retries; homepage locale 8/8) and `yarn verify`, and was fast-forward pushed to `origin/master` under the explicit owner GO; the `0fff2b59` deploy is live and the public readback confirms all 7 locales render region-appropriate offers with 0 raw keys (`/tmp/homepage-deploy-report-current`; the run's "failure" is only the known GitHub-egress `/api/health` 403 artifact). The full 467-test browser suite was stopped, not passed. Caveat: the gate is local and its provider/Auth/Storage transport is simulated, not production proof. The consented checkout journey is the remaining AC-2 item.
...
- **AC-2 (partial)** — homepage portion done: the locale fix is live and the public render shows
  region-appropriate offers with 0 raw keys in all 7 locales. Remaining: the consented real
  client → modal → checkout journey in Stripe-live with no fabricated transaction.
