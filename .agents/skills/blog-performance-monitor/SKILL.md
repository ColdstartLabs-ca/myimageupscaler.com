...
- Project-local `blog-edit` at `.Codex/skills/blog-edit/SKILL.md`: perform blog metadata/content updates after this skill produces the edit brief.
...
2. **Fetch focused GSC evidence**
   - Use `.Codex/skills/gsc-analysis` as the data source.
...
2. **Correlate with recent markdown changes/backlog**
   ...
   - Always read `.Codex/skills/blog-changelog.md` if present; it is the project-local blog edit changelog used by `blog-edit`.
...
5. **Apply updates**
   - Load `.Codex/skills/blog-edit/SKILL.md` and follow its workflow.
...
```bash
node ~/.Codex/skills/gsc-analysis/scripts/gsc-fetch.cjs --site=myimageupscaler.com --days=28 --row-limit=25000 --output=/tmp/gsc.json
```
