...
- Project-local `blog-publish` at `.Codex/skills/blog-publish/SKILL.md`: create draft, use/reuse images, publish, verify, update tracking, and append changelog.
...
1. **Read publishing context**
   - Read `.Codex/skills/blog-publish/SKILL.md` before publishing.
   - Read `.Codex/skills/blog-changelog.md` if present.
...
8. **Publish through `blog-publish`**
   - Load `.Codex/skills/blog-publish/SKILL.md`.
...
```bash
node ~/.Codex/skills/gsc-analysis/scripts/gsc-fetch.cjs --site=myimageupscaler.com --days=90 --row-limit=25000 --output=/tmp/gsc-opportunities.json
```
