...
// Analytics — GA4: mock gaSendEvent so we can assert it was called
...
// Mock useCheckoutSession to capture the onComplete callback and expose it
// so tests can trigger checkout completion without needing a real Stripe embed.
vi.mock('@client/hooks/useCheckoutSession', () => ({
  stripePromise: Promise.resolve({}),
  useCheckoutSession: (params: { onComplete: () => void }) => {
    // Capture the callback so tests can call it directly
...
describe('CheckoutModal — GA4 purchase tracking', () => {
  ...
  it('should call gaSendEvent with purchase on checkout complete', async () => {
    ...
    // Ensure the hook captured the onComplete callback
    expect(capturedOnComplete.current).toBeTypeOf('function');

    // Simulate the Stripe embedded checkout firing the onComplete callback
    await act(async () => {
