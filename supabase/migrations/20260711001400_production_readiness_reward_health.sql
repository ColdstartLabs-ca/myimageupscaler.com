-- Zero-purchase UTC days are not evidence of broken attribution. They are
-- vacuously healthy; active days still require the 95% / zero-duplicate gate.
...
AS $$
  ...
  reward_counts AS (
    ...
    GROUP BY known.purchase_id, known.experiment_key, known.assignment_key, known.assigned_arm_id
  ),
  daily_counts AS (
    SELECT
  ...
  SELECT
    ...
    round(daily_counts.attributed_count::NUMERIC / NULLIF(daily_counts.paid_count, 0), 4),
    COALESCE(daily_counts.duplicate_count, 0),
    COALESCE(daily_counts.paid_count, 0) = 0
      OR (
        daily_counts.attributed_count::NUMERIC / daily_counts.paid_count >= 0.95
...
REVOKE ALL ON FUNCTION public.get_experiment_reward_health(JSONB, TIMESTAMPTZ)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_experiment_reward_health(JSONB, TIMESTAMPTZ)
  TO service_role;
