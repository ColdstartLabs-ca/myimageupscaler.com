-- Repair email audit tables that may be marked as migrated but absent in production.
-- This is intentionally idempotent so it can run safely against fresh or drifted DBs.
...
DROP POLICY IF EXISTS "Users can view own preferences" ON public.email_preferences;
...
-- Service role bypasses RLS. Keep direct inserts closed to anon/authenticated users.
